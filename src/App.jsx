...
export default function App() {
  ...
  // --- Search Logic ---
  const filteredItems = fruits.filter(f => 
    f.text.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const exportToExcel = () => {
    if (fruits.length === 0 && sales === 0) return alert("Error: No data available for report!");
    const reportDate = new Date().toLocaleString();
    let csv = `Sultan Elite Shop Report\nDate: ${reportDate}\n\n`;
  ...
  const addFruit = () => {
    if (inputValue && inputPrice && quantity) {
      ...
      alert("Please fill all fields (Name, Price, and Quantity)!");
    }
  };
      ...
      <div style={styles.card}>
        <header style={styles.header}>
          <div>
            ...
            <p style={{fontSize: '11px', color: '#94a3b8', margin: 0}}>Inventory Management System</p>
          </div>
          <button onClick={exportToExcel} style={styles.exportBtn}>📊 Export CSV</button>
        </header>
        ...
        <div style={styles.inputCard}>
          <label style={styles.fieldLabel}>New Item Name</label>
          <input style={styles.inputField} placeholder="e.g. Honda Civic" value={inputValue} onChange={e => setInputValue(e.target.value)} />
        ...
        <div style={styles.listContainer}>
          <h3 style={{fontSize: '14px', color: '#1e293b', marginBottom: '10px'}}>Current Stock:</h3>
          {filteredItems.length === 0 && <p style={{textAlign: 'center', color: '#94a3b8'}}>No items found.</p>}
          
          {filteredItems.map(f => (
            <div key={f.id} style={styles.listItemWrapper}>
               <ListItem 
                data={f} 
                onSell={() => {
                  if (f.quantity > 0) {
                    ...
                    alert("Out of stock! Please refill.");
                  }
                }} 
              ...
              {/* Low Stock Indicator */}
              {f.quantity < 3 && (
                <div style={{color: '#ef4444', fontSize: '10px', fontWeight: 'bold', marginTop: '-10px', marginLeft: '10px'}}>
                  ⚠️ Low Stock - Please Refill
                </div>
              )}
            </div>
          ))}
        ...
        <button onClick={() => { if(window.confirm("WARNING: This will delete all your data permanently. Proceed?")) { localStorage.clear(); window.location.reload(); } }} style={styles.resetLink}>
            Permanently Reset All Shop Data
        </button>
      </div>
    </div>
  );
...
const styles = {
  ...
  searchBar: { width: '100%', padding: '12px', borderRadius: '12px', border: '2px solid #e2e8f0', outline: 'none', fontSize: '14px', boxSizing: 'border-box', backgroundColor: '#fff', color: '#000' },
  ...
  listItemWrapper: { borderBottom: '1px solid #f1f5f9', paddingBottom: '10px' },
  resetLink: { width: '100%', marginTop: '25px', background: 'none', border: 'none', color: '#94a3b8', fontSize: '11px', cursor: 'pointer', fontWeight: '700' }
};
